...
const ACCOUNT_PRODUCT_HINTS: Array<{ pattern: RegExp; product: string }> = [
  ...
  { pattern: /\b(google tasks)\b/i, product: 'Google' },
  { pattern: /\b(microsoft to do|todo)\b/i, product: 'Microsoft' },
  { pattern: /\b(google calendar)\b/i, product: 'Google' },
  { pattern: /\b(apple calendar|icloud)\b/i, product: 'Apple' },
...
const TOOL_PRODUCT_TEMPLATES: Record<string, string> = {
  ...
  calendar_list: 'Checking {product} calendar',
  calendar_list_all: 'Checking {product} calendar',
  calendar_list_sources: 'Loading {product} calendar',
